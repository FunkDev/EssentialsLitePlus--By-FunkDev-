			...
			if(config.getConfig().getString("Commands.Kill.Enabled").equalsIgnoreCase("true")) {
				if(p.hasPermission("el+.cmd.kill")) {
					if(args.length == 0) {
						...
						if(args.length == 1) {
						   if(!config.getServer().getPlayer(args[0]).isOnline()) {
				...
				p.sendMessage(ChatColor.GRAY + "[" + ChatColor.GOLD + ChatColor.BOLD + "EssentialsLite+" + ChatColor.GRAY + "] " + ChatColor.RED + "This command is disabled!");
			}
		}
		return false;
